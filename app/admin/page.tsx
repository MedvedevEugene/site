...
export default async function AdminDashboardPage() {
  let counts = { events: 0, news: 0, tariffs: 0, media: 0 };
  try {
    const [events, news, tariffs, media] = await Promise.all([
      prisma.event.count(),
      prisma.newsPost.count({ where: { published: true } }),
      prisma.tariff.count({ where: { published: true } }),
      prisma.mediaItem.count(),
    ]);
    counts = { events, news, tariffs, media };
  } catch {
    /* DB not ready */
  }
    ...
    <AdminShell>
      ...
      <AdminCard title="Быстрые действия">
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/admin/tariffs" className="btn btn-outline">Изменить тариф</Link>
