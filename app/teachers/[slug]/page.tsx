...
const TEACHERS: Record<string, { name: string; role: string; photo: string; bio: string; services: string[] }> = {
  "asya-bykovskaya": {
    name: "Ася Быковская",
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return createPageMetadata(teacher?.name || "Преподаватель");
}
...
export default async function TeacherPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <PageShell title={teacher.name} description={teacher.role} breadcrumbs={[{ label: "Преподаватели" }, { label: teacher.name }]}>
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-10">
          ...
          <div>
            <p className="text-muted mb-6">{teacher.bio}</p>
            ...
            <Link href="#contact" className="btn btn-primary">Записаться к преподавателю</Link>
          </div>
        </div>
      </PageShell>
